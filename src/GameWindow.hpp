...
void loadDataFromJson(vector<Question> &questions)
{
    ...
    for (const auto &filename : database)
    {
        ...
        try
        {
            ...
            {
                cout << "Warning: Not enough questions in " << filename << ". Need " << questionsToPickPerFile << " but only found " << data.size() << endl;
                continue;
            }
            ...
            {
                cout << "Warning: Could only pick " << usedIndexes.size() << " questions from " << filename << " instead of the required " << questionsToPickPerFile << endl;
            }
        }
